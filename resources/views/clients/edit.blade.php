...
@section('content')
    <div class="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-5 duration-500">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-black tracking-tight dark:text-gray-100 flex items-center gap-3 italic">
                    <i data-lucide="edit-3" class="w-8 h-8 text-primary"></i> Perfil de Cliente
                </h1>
                <p class="text-sm text-gray-500 dark:text-gray-400 font-medium">Actualizá la información personal y de
                    contacto del cliente.</p>
            </div>
        ...
        <!-- Form Card -->
        <div
            class="bg-white dark:bg-dark-alt rounded-[40px] border border-gray-100 dark:border-white/5 shadow-2xl shadow-gray-200/50 dark:shadow-none p-8 md:p-12 relative overflow-hidden">

            <div class="absolute right-0 top-0 w-32 h-32 bg-primary/5 rounded-bl-[100px]"></div>

            <form action="{{ route('client.update', $client) }}" method="POST" class="space-y-8 relative">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="md:col-span-2 space-y-2">
                        ...
                        <select name="price_list_id"
                            class="w-full px-6 py-4 rounded-2xl bg-gray-100 dark:bg-dark border border-transparent focus:border-primary/50 focus:bg-white dark:focus:bg-dark-alt outline-none transition-all duration-300 font-bold">
                            <option value="">— Sin lista (precio {{ old('client_type', $client->client_type?->value) == 'wholesale' ? 'mayorista' : 'minorista' }} estándar) —</option>
                            @foreach($priceLists as $priceList)
                                <option value="{{ $priceList->id }}" {{ old('price_list_id', $client->price_list_id) == $priceList->id ? 'selected' : '' }}>{{ $priceList->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="md:col-span-2 space-y-2">
                        ...
                        <textarea name="notes" rows="3"
                            class="w-full px-6 py-4 rounded-2xl bg-gray-100 dark:bg-dark border border-transparent focus:border-primary/50 focus:bg-white dark:focus:bg-dark-alt outline-none transition-all duration-300 font-bold"
                            placeholder="Cualquier detalle relevante del cliente...">{{ old('notes', $client->notes) }}</textarea>
                    </div>
