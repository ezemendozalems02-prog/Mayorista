...
<div class="max-w-2xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-5 duration-500 pb-20">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-black tracking-tight dark:text-gray-100 flex items-center gap-3 italic">
                <i data-lucide="edit-3" class="w-8 h-8 text-violet-500"></i> Editar Sucursal
            </h1>
            <p class="text-sm text-gray-500 dark:text-gray-400 font-medium">Modificá los datos de <strong>{{ $branch->name }}</strong>.</p>
        </div>
    ...
    <!-- Form -->
    <form action="{{ route('branch.update', $branch) }}" method="POST" class="space-y-6">
        @csrf @method('PATCH')
        ...
        <div class="bg-white dark:bg-dark-alt rounded-[40px] border border-gray-100 dark:border-white/5 shadow-2xl shadow-violet-500/5 p-8 md:p-10 space-y-6">
            ...
            <div class="flex items-center justify-between p-5 bg-gray-50 dark:bg-dark rounded-2xl">
                <div>
                    ...
                    <p class="text-xs text-gray-400 font-medium mt-0.5">Las sucursales inactivas no aparecen en los filtros.</p>
                </div>
                <label class="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" {{ $branch->is_active ? 'checked' : '' }} class="sr-only peer">
            ...
            <div class="space-y-2">
                ...
                <textarea name="notes" rows="3"
                    class="w-full px-6 py-4 rounded-2xl bg-gray-50 dark:bg-dark border border-transparent focus:border-violet-500/50 focus:bg-white dark:focus:bg-dark-alt outline-none transition-all duration-300 font-bold resize-none">{{ old('notes', $branch->notes) }}</textarea>
            </div>
        </div>
