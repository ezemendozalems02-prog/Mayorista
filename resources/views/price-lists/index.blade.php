        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div class="relative w-full md:w-80 group">
                <form action="{{ route('price-list.index') }}" method="GET">
                    ...
                    <input name="search" value="{{ request('search') }}" type="text" placeholder="Buscar lista de precios..."
                ...
                <table class="w-full">
                    <thead>
                        <tr class="bg-gray-50/50 dark:bg-white/5 border-b border-gray-100 dark:border-white/5 text-[10px] text-gray-400 font-black uppercase tracking-[0.2em] italic">
                            <th class="px-8 py-5 text-left">Lista</th>
                            <th class="px-8 py-5 text-left hidden sm:table-cell">Productos con Precio Especial</th>
                            <th class="px-8 py-5 text-left hidden sm:table-cell">Clientes Asignados</th>
                    ...
                    <tbody class="divide-y divide-gray-50 dark:divide-white/5">
                        @forelse($priceLists as $priceList)
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-white/5 transition-all group">
                                <td class="px-8 py-6">
                                    <p class="text-sm font-black text-gray-900 dark:text-gray-100 italic tracking-tight">{{ $priceList->name }}</p>
                                    @if($priceList->description)
                                        <p class="text-[10px] text-gray-400 font-bold mt-1 italic">{{ \Illuminate\Support\Str::limit($priceList->description, 60) }}</p>
                                    @endif
                                </td>
                                ...
                                <td class="px-8 py-6 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('price-list.show', $priceList) }}"
                                        ...
                                        <a href="{{ route('price-list.edit', $priceList) }}"
                                        ...
                                        <form action="{{ route('price-list.destroy', $priceList) }}" method="POST" onsubmit="return confirm('¿Eliminar esta lista de precios?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                            ...
                            <tr>
                                <td colspan="5" class="py-20 text-center">
                                    <div class="flex flex-col items-center justify-center text-gray-400">
                                        <i data-lucide="tags" class="w-16 h-16 mb-4 opacity-20"></i>
                                        <p class="text-lg font-black tracking-tight italic">No hay listas de precios todavía</p>
                                        <p class="text-xs uppercase font-bold mt-1 opacity-50 tracking-widest">Creá una para ofrecer precios especiales a ciertos clientes</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
