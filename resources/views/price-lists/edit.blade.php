...
@section('content')
    <div class="max-w-2xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-5 duration-500">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-black tracking-tight dark:text-gray-100 flex items-center gap-3 italic">
                    <i data-lucide="edit-3" class="w-8 h-8 text-primary"></i> Editar Lista
                </h1>
                <p class="text-sm text-gray-500 dark:text-gray-400 font-medium">Para cargar precios de productos, andá a "Precios" desde el listado.</p>
            </div>
        ...
        <div class="bg-white dark:bg-dark-alt rounded-[40px] border border-gray-100 dark:border-white/5 shadow-2xl shadow-gray-200/50 dark:shadow-none p-8 md:p-12">
            <form action="{{ route('price-list.update', $priceList) }}" method="POST" class="space-y-8">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="description" rows="3"
                        class="w-full px-6 py-4 rounded-2xl bg-gray-100 dark:bg-dark border border-transparent focus:border-primary/50 focus:bg-white dark:focus:bg-dark-alt outline-none transition-all duration-300 font-bold">{{ old('description', $priceList->description) }}</textarea>
                </div>

                <div class="flex items-center gap-3">
                    <input type="checkbox" name="is_active" value="1" id="is_active" {{ old('is_active', $priceList->is_active) ? 'checked' : '' }}
                        class="w-5 h-5 rounded-lg text-primary focus:ring-primary">
                    <label for="is_active" class="text-xs font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest">Lista activa</label>
                </div>
