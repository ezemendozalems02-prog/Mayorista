...
@section('content')
    <div class="max-w-2xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-5 duration-500">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-black tracking-tight dark:text-gray-100 flex items-center gap-3">
                    <i data-lucide="tags" class="w-8 h-8 text-primary"></i> Nueva Lista de Precios
                </h1>
                <p class="text-sm text-gray-500 dark:text-gray-400 font-medium">Después de crearla vas a poder cargar precios especiales por producto.</p>
            </div>
        ...
        <div class="bg-white dark:bg-dark-alt rounded-[40px] border border-gray-100 dark:border-white/5 shadow-2xl shadow-gray-200/50 dark:shadow-none p-8 md:p-12">
            <form action="{{ route('price-list.store') }}" method="POST" class="space-y-8">
                @csrf
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="description" rows="3" placeholder="Para qué tipo de cliente es esta lista"
