        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div class="relative w-full md:w-80 group">
                <form action="{{ route('brand.index') }}" method="GET">
                    ...
                    <input name="search" value="{{ request('search') }}" type="text" placeholder="Buscar marca..."
                ...
                <table class="w-full">
                    <thead>
                        <tr class="bg-gray-50/50 dark:bg-white/5 border-b border-gray-100 dark:border-white/5 text-[10px] text-gray-400 font-black uppercase tracking-[0.2em] italic">
                            <th class="px-8 py-5 text-left">Marca</th>
                    ...
                    <tbody class="divide-y divide-gray-50 dark:divide-white/5">
                        @forelse($brands as $brand)
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-white/5 transition-all group">
                                <td class="px-8 py-6">
                                    <p class="text-sm font-black text-gray-900 dark:text-gray-100 italic tracking-tight">{{ $brand->name }}</p>
                                </td>
                                ...
                                <td class="px-8 py-6 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        <form action="{{ route('brand.destroy', $brand) }}" method="POST" onsubmit="return confirm('¿Eliminar esta marca?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                            ...
                            <tr>
                                <td colspan="4" class="py-20 text-center">
                                    <div class="flex flex-col items-center justify-center text-gray-400">
                                        <i data-lucide="tag" class="w-16 h-16 mb-4 opacity-20"></i>
                                        <p class="text-lg font-black tracking-tight italic">No hay marcas todavía</p>
                                        <p class="text-xs uppercase font-bold mt-1 opacity-50 tracking-widest">Creá la primera marca de tu catálogo</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
