...
<head>
    ...
    <title>Login - {{ config('platform.brand.name') }}</title>
...
<body class="min-h-screen bg-dark flex flex-col justify-center py-8 md:py-12 px-4">
    ...
    <div class="w-full max-w-md mx-auto">
        ...
        <div
            ...
            @if(auth()->check() && auth()->user()->is_demo)
                <div class="mb-6 p-4 bg-amber-500/10 border border-amber-500/20 text-amber-500 rounded-2xl text-[10px] md:text-xs font-bold text-center uppercase tracking-widest">
                    ...
                    Sesión Demo Activa. Logueate con tus datos reales.
                </div>
            @endif

            @if (session('status'))
                <div class="mb-6 p-4 bg-green-500/10 border border-green-500/20 text-green-500 rounded-2xl text-xs font-bold text-center animate-pulse">
                    {{ session('status') }}
                </div>
            @endif
            ...
            <form action="{{ route('login.post') }}" method="POST" class="space-y-5 md:space-y-6">
                ...
                <div class="space-y-2" x-data="{ show: false }">
                    <div class="flex justify-between items-center px-1">
                        <label
                            class="text-xs font-bold text-gray-400 uppercase tracking-widest">Contraseña</label>
                        <a href="{{ route('password.request') }}" class="text-[10px] font-black text-primary hover:underline">¿Olvidaste tu clave?</a>
                    </div>
                    <div class="relative group">
                        <i data-lucide="lock"
                            class="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 group-focus-within:text-primary transition-colors"></i>
        ...
        <p
            ...
            {{ config('platform.brand.name') }} &copy; {{ date('Y') }}
        </p>
